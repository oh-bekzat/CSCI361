...
console.log("idddd = ", selectedMaintenanceId);

const AdminUpdateMaintenance = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const userResponse = await axios.get(`http://localhost:3001/users/${selectedMaintenanceId}`);
  ...
  const handleSubmit = (e) => {
    ...
    if (validateForm()) {
      const apiUrl = `http://localhost:3001/users/${selectedMaintenanceId}`;
...
export default AdminUpdateMaintenance;
